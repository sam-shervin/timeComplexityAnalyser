...
#define INT_MAX_NUM 999999999 // Using the standard INT_MAX causes an error
                              ...
                              // since the value gets too large when added to
                              // another value.
...
int main()
{
    ...
    vector<int> numPoints = {5000, 10000, 50000, 100000, 500000, 1000000};
    vector<double> tX_values, tY_values, tXY_values;
    for (int i = 0; i < numPoints.size(); i++)
    {
        ...
        tY = computeTime(points, 2);
        tY_values.push_back(tY);
        cout << "t(n) for sorting " << numPoints[i] << " points based on y-coordinate: " << tY << " seconds" << endl;
        tXY = computeTime(points, 3);
        tXY_values.push_back(tXY);
        cout << "t(n) for sorting " << numPoints[i] << " points based on average of x and y coordinates: " << tXY << " seconds" << endl;
    }

    // plot the results and display the coordinates

    plt::figure_size(1200, 780);
    plt::plot(numPoints, tX_values, {{"label", "t(n) for sorting based on x-coordinate"}});
    plt::plot(numPoints, tY_values, {{"label", "t(n) for sorting based on y-coordinate"}});
    plt::plot(numPoints, tXY_values, {{"label", "t(n) for sorting based on average of x and y coordinates"}});
    // display coordinates in the format (x, y) in figure  by using plt::annotate
    for (int i = 0; i < numPoints.size(); i++)
    {
        plt::annotate("(" + to_string(numPoints[i]) + ", " + to_string(tX_values[i]) + ")", numPoints[i], tX_values[i]);
        plt::annotate("(" + to_string(numPoints[i]) + ", " + to_string(tY_values[i]) + ")", numPoints[i], tY_values[i]);
        plt::annotate("(" + to_string(numPoints[i]) + ", " + to_string(tXY_values[i]) + ")", numPoints[i], tXY_values[i]);
    }
    plt::title("Time Complexity of Merge Sort for 2D Points");
